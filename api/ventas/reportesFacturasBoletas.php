...
if($_SERVER['REQUEST_METHOD'] == 'OPTIONS'){
    ...
    if($exito){
        $ventaC = new Venta($db);                                      
        $datos = $ventaC->listarVentasMesAnterioryActual($mensaje,$code_error,$exito);

        if($exito==true){
            header('HTTP/1.1 200 OK');
            echo json_encode( array("error"=>$code_error, "MES_ACTUAL"=>$datos['MES_ACTUAL'], "MES_ANTERIOR"=>$datos['MES_ANTERIOR'], "mensaje"=>$mensaje,"exito"=>true));
        }else{
