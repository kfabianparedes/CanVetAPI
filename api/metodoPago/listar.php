<?php
    ...
    if($exito){
        $_exito = false;
        $metodos = array();
        $metodo = new MetodoPago($db);
        $metodos = $metodo->listarMetodosDePago($mensaje,$code_error,$_exito);
        if($_exito){
            header('HTTP/1.1 200 OK');
            echo json_encode( array("error"=>$code_error, "resultado"=>$metodos, "mensaje"=>$mensaje,"exito"=>true));
        }else{
            header('HTTP/1.1 400 Bad Request');
            echo json_encode( array("error"=>$code_error, "resultado"=>$metodos, "mensaje"=>$mensaje,"exito"=>false));
        }
    }