<?php
    ...
    if($exito){
        if(esValido($mensaje)){
            ...
            $detVenta->VENTA_ID = $_GET['VENTA_ID'];
            $datos = $detVenta->listarDetallesPorVenta($mensaje, $code_error,$exito);
    ...
    function esValido(&$m){ 
        ...
        if(!isset($_GET['VENTA_ID'])){
            ...
            if($_GET['VENTA_ID'] == ''){
                $m = "El campo VENTA_ID no puede estar vacío o ser null.";
                return false;
            }else{
                if(!is_numeric($_GET['VENTA_ID'])){
