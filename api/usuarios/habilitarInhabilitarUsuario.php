<?php
    ...
    header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, User, X-Requested-With");
    ...
    if($exito){
        if(esValido($data,$mensaje)){
            $usuarioC->USU_ID = $data->USU_ID;
            $usuarioC->USU_ESTADO = $data->USU_ESTADO;
            $exito =  $usuarioC->habilitarInhabilitarUsuario($mensaje,$code_error);
            if($exito == true)
    ...
    function esValido($d,&$mensaje){
        ...
        if(is_null($d)){
            $mensaje = "Los datos ingresados deben respetar el formato json";
            return false;
        }else{
            if(!isset($d->USU_ID)){
                $mensaje = "la variable USU_ID no ha sido enviada.";
                return false;
            }else{  
                if($d->USU_ID == ""){
                    $mensaje = "la variable USU_ID no puede estar vacía o ser null.";
                    return false; 
                }else{
                    if(!is_numeric($d->USU_ID)){
                       $mensaje = "la variable USU_ID solo acepta caracteres numéricos.";
                       return false;  
                    }else{
                        if($d->USU_ID < 1 ){
                            $mensaje = "la variable USU_ID no puede ser menor o igual a 0.";
                            return false; 
                        }
                    }
                }
            ...
            if(!isset($d->USU_ESTADO)){
                $mensaje = "la variable USU_ESTADO no ha sido enviada.";
                return false;
            }else{  
                if($d->USU_ESTADO == "" && $d->USU_ESTADO!=0 ){
                    $mensaje = "la variable USU_ESTADO no puede estar vacía o ser null.";
                    return false; 
                }else{
                    if(!is_numeric($d->USU_ESTADO)){
                       $mensaje = "la variable USU_ESTADO solo acepta caracteres numéricos.";
                       return false;  
                    }else{
                        if($d->USU_ESTADO < 0  || $d->USU_ESTADO > 1){
                            $mensaje = "la variable USU_ESTADO no puede ser menor a 0 o mayor a 1.";
                            return false; 
                        }
                    }
                }
